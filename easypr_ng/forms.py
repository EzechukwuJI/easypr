...
class ContentUploadForm(forms.ModelForm):
	...
	person_to_quote          =    forms.CharField(max_length = 128, widget=forms.TextInput(attrs={'class' : 'form-control no-border-radius bg-white', 'placeholder': 'Person to quote', 'required':'required'}))
	persons_position         =    forms.CharField(max_length = 128, widget=forms.TextInput(attrs={'class' : 'form-control no-border-radius bg-white', 'placeholder': 'Position', 'required':'required'}))
